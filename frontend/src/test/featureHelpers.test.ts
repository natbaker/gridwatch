...
describe('buildGapSeries', () => {
  ...
  it('returns empty data when there are no events', () => {
    const { data, keys } = buildGapSeries([], drivers)
    expect(data).toEqual([])
    expect(keys.map(k => k.abbreviation)).toEqual(['VER', 'HAM'])
  })

  it('buckets and carries forward latest gap per driver', () => {
    const events = [
      { t: 0, n: 1, g: 0, i: 0 },
      { t: 0, n: 44, g: 1.5, i: 1.5 },
      { t: 30, n: 44, g: 2.2, i: 0.7 },
    ]
    const { data } = buildGapSeries(events, drivers, 30)
    expect(data[0]).toMatchObject({ t: 0, VER: 0, HAM: 1.5 })
    expect(data[1]).toMatchObject({ t: 30, VER: 0, HAM: 2.2 })
  })
})
